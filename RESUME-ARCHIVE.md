# RESUME-ARCHIVE — Session-Historie

Historie ausgelagert aus RESUME.md am 2026-07-20. Chronologisch, neueste zuerst.

---

# Designbridge — Schnellstart-Spickzettel

Stand: **20.07.2026 vormittags (✅ BRAND-ROLLBACK AUF ZINK LIVE `c63114c` + ORCHESTRIERTER TEST 6/6 GRÜN · davor: Rebrand-Skin + Bild-Zuverlässigkeit · Breiten-Test · Architektur-Pivot Scheibe 1–3 · Figma-Reverse-Import v1)** — **🚀 APP LIVE: https://designbridge-production.up.railway.app** mit **Gemini PAID**, UIPrism-Name/Favicon bleibt, aber **zurück im zink-Look** (kein Indigo/Flieder/Spektrum). Server **289/289** · Web **584/584** · Plugin **98/98**.

## 🎨 UI-FEINSCHLIFF SESSION 20.07. nachmittags (Screen-by-Screen mit Rob, alles LIVE gepusht)

Rob und Claude sind live durch die App gegangen und haben Screen für Screen gefeilt. **Arbeitsmodus (Robs Wunsch): lokal bauen → in der internen Browser-Vorschau zeigen → am Schluss gebündelt pushen** (NICHT mehr zwischendrin deployen). Trick fürs lokale Preview MIT Daten: Prod-`localStorage['designbridge.lastImport']` per `window.name` (übersteht Cross-Origin-Nav) in den lokalen Vite-Tab kopiert. **Lokaler Vite läuft auf Port 5199** (`npx vite --port 5199`, kein Express/API — für reine UI-Vorschau; URL/Bild-Import geht dort nicht, Daten injizieren).

**Header/Sidebar (live):** App-Icon aus Brand-Katalog (`web/public/uiprism-appicon.svg`, indigo Kachel, Prisma vergrößert+geclippt, 18px) statt diffusem Mark; Wortmarke gesperrt; `v0.1.1` auf Grundlinie; „Settings"→**„Einstellungen"** + neuer Platzhalter **„Verlauf"** (helle Outline); Warnungen/Systemmeldungen als **rosa Pill, horizontal in der Bar zentriert** (`serverOk===false`); Sidebar-IA: **Dashboard · ─ · Tokens/Atoms/Molecules/Organisms/Templates · ─ · Export** (Export=Peer), „LIBRARY"-Label raus. **Wording-Regel (Rob):** systemische Begriffe (Atoms/Molecules/Tokens/Components) englisch, beschreibende UI-Texte deutsch.

**Dashboard-Inhalt (live):** „Komponenten-Erkennung verfeinern"-Button schwarz→**Terracotta `#c2553d`** (stimmt aufs amber-Infofenster ab, sekundär); Info-Labels dunkler (zinc-500); **Tokens + UI-Inventory nebeneinander (2 Spalten)**; AiDeepenBanner auf `max-w-3xl` (fluchtet mit Übersicht-Inhalt).

**Tokens-Seite (live, `web/src/pages/Tokens.jsx` + `components/library/tokenViews.jsx`):** Farbname (Großbuchstabe, informell) ÜBER der Kachel; Herkunft „↳ --var"→**„CSS var"** (Grotesk, ohne `--`-Präfix, lesbar); **alles Grotesk** (Monospace raus); Typografie mit **größenbasierter Hierarchie Display/Heading/Copy/Caption** (`assignHierarchy()`, größenabgeleitet NICHT semantisch, groß→klein sortiert, Label prominent); **Spacing·Border-Radius·Shadows nebeneinander in Spalten mit vertikalen Trennlinien** (`border-l`+`pl-8`); Farbraster 6-spaltig. App-Shell: **sichtbare Scrollbar** (`.db-scroll` in index.css) + **Footer** (UIPrism v0.1.1 + Tagline „Map your UI, automatically."). Web 584/584.

**⏭️ OFFEN fürs nächste Mal:** (F) gelber Info-Kasten-Ton (bewusst gelassen); (G) „Quelle: url"-Platzierung (Rob hinterfragt noch); (T-propagate) dieselben Kachel-/Listen-Muster auf **Atoms/Molecules/Organisms/Templates** übertragen — ⚠️ ANDERE Komponenten (LibraryObjectList, nicht tokenViews), eigener Umbau, „andere Sachen erst wichtiger"; Farb-Spaltenzahl (6) + Farbname-Größe evtl. nachjustieren. Recursive-Decomposition-Idee bleibt auf Branch `experiment/rekursive-zerlegung` (post-Präsentation).

## ✅ BRAND-ROLLBACK AUF ZINK + PRESENTATION-READY-TEST 6/6 GRÜN (20.07. vormittags, `c63114c`) — Robs Feedback: Rebrand gefiel nicht

**Rollback (`c63114c`, web-only, 3 Dateien):** Robs Kritik am UIPrism-Skin — Indigo/Flieder/Spektrum-Leiste raus, zurück zum zink/weiß-Look; **Name/Favicon/Title UIPrism bleiben** (nur Farben/Linie/Logo-Spacing). Umgesetzt OHNE 20 Datei-Patches: `primary`-Token in `tailwind.config.js` auf zink retargt (`DEFAULT #18181b`, hover `#3f3f46`, soft `#f4f4f5`, ink `#18181b`), `spectrum`-Tokens gelöscht → alle `bg-primary`/`primary-soft`/`text-primary` überall schwarz/grau (Buttons, aktive Nav, Wortmarke). Gradient-Div aus dem Header entfernt (graue `border-b` = schlichte Linie wie früher). `uiprism-mark.svg` viewBox von `0 0 185 118.6` auf Inhaltsgrenzen `0 2 173 83` gestrafft → Logo vertikal exakt zentriert (Offset 0, war 30% Leerraum unten). Web **584/584** grün, Browser+Prod-verifiziert (aktive Nav `rgb(244,244,245)`=zinc-100, kein Spektrum-Div). ⚠️ Test-Leitfaden §1 auf zink nachgezogen.

**Orchestrierter Test (Leitfaden `docs/2026-07-20-test-leitfaden-demo.md`, autonom auf Prod, Rob klickte nichts): ALLE 6 GRÜN → Presentation-Ready-Gate erfüllt.**
- **①** Branding-Smoke: schwarz/grau, kein Flieder/Spektrum, Deploy `c63114c` live ✅
- **②** Bild-Import großer Screenshot (Reliability-Fix): `Testdaten/Bildschirmfoto 2026-07-15 um 17.46.11.png` **3200×2258** per `POST /api/scan/image` an Prod → HTTP 200, 32,6s, `model:gemini-3.5-flash` (echt, kein Fallback), `warnings:[]`, volle Shape (tokens 6/4/3 + atoms/molecules/organisms/templates + composition). **Der früher brechende Fall läuft jetzt sauber** = Downscale+Retry bewiesen (1 Gemini-Call). `image_width:3200` bleibt im meta (Downscale nur in-memory) ✅
- **③** Library: Tokens/Atoms(3)/Molecules(1)/Organisms(6, KI-interpretiert gemini-3.5-flash)/Templates(1); Footer aufgeklappt = Vorschau rendert + plan-abgeleiteter Tailwind (`bg-on-primary`, `text-base font-base` + arbitrary Fallbacks) ✅
- **④** Figma-Export 1:1 via `figma-e2e-test`-Skill AUTONOM: Payload gestaged (`An Figma senden` → Prod `/api/figma-export`, 11 components), Figma Desktop per AppleScript ferngesteuert, Plugin-Erfolg „8 Farben, 4 Textstile, 11 Bausteine (3/1/6/1) · 2 übersprungen". MCP-verifiziert Datei **`sSt4fOZRtykBfDfUAZ9f99`** („Ohne Namen"): Sektionen DB/Atoms(Button 3 Var/Input 2/Badge 2)·Molecules(Suche)·Organisms(Formular/Liste/Card/Navbar/Hero/Footer)·Templates(Page Layout 480×363), Screenshot zeigt echten Inhalt (Liste „Punkt 1-3", Card „Karte A/Neu", Hero „ACME Analytics"+Buttons, Footer „© ACME"), dunkelgrün #022d2c-Tokens korrekt ✅
- **⑤** Code-Export: CSS-Variablen + Tailwind-Config Vorschau token-treu, Format-Umschalter läuft, „Storybook (folgt)" ehrlich disabled ✅
- **⑥** URL-Import (Fallback): Demo-Report → „Import complete", 23 Tokens (8/4/3/5/3) + 11 Inventar, deterministisch/gratis ✅

**Dokumentierte, NICHT-blockierende Rest-Punkte:** (a) „Suche"-Component-Ref löst beim Figma-Import nicht auf → 2 leere Fallback-Kästchen (Formular oben + Navbar). (b) Export-Code-Header + URL/Repo heißen noch „DesignBridge" (Umbenennung bewusst zuletzt). (c) Bild-Test lief über den Server-Endpoint (wo der Fix sitzt), NICHT den vollen UI-Upload+Interpret-Flow — den vollen Klickweg mit echtem Material kann Rob bei Bedarf einmal selbst eyeballen.

**⏭️ NÄCHSTES:** Gate erfüllt → sauberer Recording-Fallback-Take (Proben 24.–27.07.), Deck/One-Pager-Screenshots auf zink-Stand erneuern. UI-Layout-Redesign + Umbenennung bleiben Post-Präsentation.
...
## 📌 POST-PRÄSENTATION-IDEE (NEU 20.07., aus Robs Bild-Import-Test) — REKURSIVE ZERLEGUNG / „Kleinteile aus Organismen ziehen"

**Befund (Robs Test, Figma-Datei `qfLsiW2wWrm6LLL2rxH0fM`, Executive-Dashboard):** Nur 3 Atoms (Logo, Notification Icon, Badge), keine Buttons. **Ursache belegt am Plan-Baum:** Der Bild-Weg listet die *eigenständigen Top-Level*-Bausteine je Ebene, zerlegt Organismen aber **nicht rekursiv**. Die 9 Sidebar-Nav-Items + KPI-Card-Innereien (Label/Wert/Trend-Badge/Icon) sind im Baum vorhanden, aber als anonyme `box`/`text`/`svg`-Kinder *innerhalb* des Organismus — nicht als eigene Library-Atome/Moleküle herausgelöst. Atomic-Taxonomie-Spec (18.07.) behandelt Items bewusst als FLACHE Liste (Guard sortiert nur um, hebt nichts heraus) → **rekursive Zerlegung war NIE im Plan, ist neue Idee.** Gegenbeweis „kein Tool-Limit": Signup-Screen lieferte 11 Atoms inkl. 3 Buttons.

**Idee = Feature (Option B):** Organismen in wiederverwendbare Sub-Bausteine zerlegen (z.B. „Sidebar Nav Item" Icon+Label aktiv/inaktiv, „KPI-Card"-Muster). Substanziell: Prompt + Decompose-Logik + Dedup gleicher Atome über Organismen + Figma-Emit-Folgen + Tests + Quota. **ENTSCHEIDUNG (Rob): NICHT vor der Präsentation (29.07.) bauen — Demo-Gate grün halten.** Wird auf separatem Experiment-Branch erprobt (s.u.), Main bleibt für Proben/Deck.

**⚙️ ZWEI-STRÄNGE-WORKFLOW (Robs Wunsch 20.07.):** `main` = Präsentations-Strang (Proben, Deck, kleine Bugs) — Live-App hängt NUR an main. Experiment-Branch **`experiment/rekursive-zerlegung`** = Sandbox für Option B; Branch-Push löst KEIN Railway-Deploy aus (nur main deployt). Experiment idealerweise in EIGENER Claude-Session/Worker fahren, damit die zwei Stränge sich nie in die Quere kommen. Zurückführen nach main = bewusster Extra-Schritt, frühestens nach dem 29.

**Kleine Polish-Kandidaten (echtes Schleifen, kein Feature, aus demselben Test-JSON):** (1) `Revenue Evolution Chart` kam als Platzhalter (nicht interpretiert). (2) Off-Canvas-Ausreißer: „Figma Make/Remix"-Leiste im Template auf `absolute.x≈298041` (Quell-Wasserzeichen des Screenshots → auch der „Figma Remix Badge"-Atom stammt daher, ist kein echtes App-UI).
