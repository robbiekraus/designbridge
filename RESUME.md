...
**Robs Testprogramm:** ⚠️ **Dev-Plugin in Figma NEU LADEN** (Plugin-dist neu gebaut wg. Taxonomie!) → App neu laden → Bild-Import → Plugin-Import in **leere** Seite/Datei. Zwei Dinge zu prüfen: **(A) Layout** (Stretch&Grow + Wurzel-Freeze): Sidebar oben+volle Höhe, Monats-Labels verteilt, Titel/Wert getrennt. **(B) Taxonomie**: Figma-Sektionen heißen jetzt `DB/Atoms`, `DB/Molecules`, `DB/Organisms`, `DB/Templates`; Cards/Charts/Tabelle/Sidebar landen in **Organisms**, der ganze Screen als **1 Template**, Buttons/Icons in **Atoms**, Suchfeld/Listeneintrag in **Molecules**. In der App: Sidebar-Nav = Tokens/Atoms/Molecules/Organisms/Templates. Claude verifiziert per Figma-MCP (Datei-Link geben). Bekannt offen: Tabellen-Spaltenraster (eigene Scheibe).
