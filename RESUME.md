...
Stand: **18.07.2026 mittags (Pattern-Fidelity-Scheibe „Stretch & Grow" FERTIG, s. unten)** — **🚀 APP IST LIVE: https://designbridge-production.up.railway.app** mit **Gemini PAID** (Google Cloud Billing seit 17.07., kein Quota-Engpass mehr). Server **208/208** · Web **440/440** · Plugin **92/92**.

## ⏭️ WIEDEREINSTIEG NÄCHSTE SESSION (Robs Figma-Test der Stretch&Grow-Scheibe)

**Robs Testprogramm:** ⚠️ **Dev-Plugin in Figma NEU LADEN** (dist neu gebaut!) → App neu laden → Bild-Import (oder gespeicherten Import erneut „An Figma senden") → Plugin-Import in **leere** Seite/Datei. In Figma erwartet (die 3 test6-Pattern-Befunde): **Sidebar beginnt OBEN und füllt die volle Höhe** (schwebt nicht mehr), **Monats-Labels über die Chartbreite verteilt** (nicht mehr „DecJanFeb…" mittig), **Top-Emissions: Titel links / Wert rechts** (keine Überlappung). Bekannt offen bleibt NUR das Tabellen-Spaltenraster (eigene Scheibe). Claude kann das Figma-Ergebnis per Figma-MCP selbst verifizieren (Rob muss nur die Datei im Desktop offen haben; Link geben).
...
0. **🏷️ TAXONOMIE-ENTSCHEIDUNG (Robs Kernkritik aus test 7, WARTET AUF ROB):** Die Ebenen-Zuordnung ist aus Designer-Sicht falsch — Cards/Charts/Tabellen laufen als „component", müssten aber **Patterns** sein; **Komponenten** sind Button, Dropdown-Feld, Pulldown-Menü etc.; die ganze nachgebaute Seite darf nicht „das Pattern" sein („da würde jeder Designer das kritisieren"). Payload-Beleg vom 18.07.: 3 atomics (Badge/Search/Dropdown) · 9 components (alle Cards+Charts+Tabelle!) · 1 pattern (= ganze Seite). Rob hat die Modell-Frage (4 Ebenen inkl. Screens vs. 3 Ebenen-Varianten) noch NICHT beantwortet (Frage 18.07. dismissed) — **zuerst Robs Zielmodell klären**, dann Scheibe: KI-Scan-Prompt + recognizeComponents-Regeln + UI-Sektionen + Plugin-Sektionen/kind-Enum.
1. **Robs Figma-Beweis-Test nach dem Wurzel-Freeze-Nachfix** (s. Wiedereinstieg oben) + Rest-Scheibe **Tabellen-Spaltenraster**.
