...
>
> **🏷️ NAMENSENTSCHEIDUNG: „Refracta" — VORLÄUFIG, Rob schläft drüber (14.07. abends).** Doku mit Markt, Verworfenen & Verfügbarkeits-Checks: `docs/2026-07-14-naming-positionierung.md`. ⛔ **Umbenennung NICHT starten, bevor Rob den Namen bestätigt.** Nach Bestätigung: (Rob) Domains `refracta.design`/`.dev` registrieren → dann technische Umbenennung (Repo, README, package.json, UI-Branding, CLAUDE.md) als eigenes Arbeitspaket.
>
...
3. ✅ **ERLEDIGT (`bcd5164`) — die drei Kleineren:** leerer Datei-Inhalt = kein Material (`repoDecomposer` → structure null, kein `sourceCode:''`); `decompose` Default-cap = `CODE_CAP` (Interpret-Route rief ohne Optionen → ungecappter Prompt); Batch nimmt nur noch gehobene Bausteine (Patterns/pfad-only raus, gehobene Template-Namens-Kollisionen wie `CardSkeleton` rein — FF2-Konsistenz). Server 138/138 · Web 303/303, Browser-verifiziert (taxonomy: Zähler=67=exakt die Gehobenen).
